...
2，将压缩文件解压（zip文件）
 */
func (tomcat TomcatType) PullPackageInfo(param *model.StartApp) error {
	...
	packagePathTemp := fmt.Sprintf("/data/backup/%v/tomcat/%v", param.BuildType, param.RecordId)
	...
	//	return err
	//}
	warFileName := fmt.Sprintf("%v/%v", param.PackagePathTemp, param.PackageName)
	cmd = fmt.Sprintf("cd %v && source /etc/profile && jar -xvf %v", param.PackagePathTemp, param.PackageName)
	_, err = utils.ExecCommand(cmd)
	if err != nil {
		utils.Logger().Warn("解压%v war文件fail:%v", warFileName, err)
		return err
	}
...
func (tomcat TomcatType) ReStart(param *model.StartApp) error {
	err := tomcat.Stop(param)
	if err != nil {
		return err
	}
	err = tomcat.Clear(param)
	if err != nil {
		return err
	}
	err = tomcat.MvFile(param)
	if err != nil {
		return err
	}
	err = tomcat.Start(param)
	return err
}
func (tomcat TomcatType) Start(param *model.StartApp) error {
	cmd := "source /etc/profile"
	_, err := utils.ExecCommand(cmd)
	if err != nil {
		utils.Logger().Error("source /etc/profile fail:%v", err)
	...
	cmd = fmt.Sprintf("nohup %v/bin/startup.sh &", param.ContainerPath)
	utils.Logger().Info("开始启动app:%v", param.PackageName)
	err = tomcat.StartCmd(param, cmd)
	if err != nil {
		return err
	}
	utils.Logger().Info("slepp 4s...............")
	time.Sleep(4 * time.Second)
	pid, err := tomcat.FindPid(param.ContainerPath)
	if err != nil {
...
4,kill -9 pid
 */
func (tomcat TomcatType) Stop(param *model.StartApp) error {
	pid, err := tomcat.FindPid(param.ContainerPath)
	if err != nil {
		return err
	}
	...
	cmd := fmt.Sprintf("source /etc/profile && %v/bin/shutdown.sh", param.ContainerPath)
	...
	pidTemp, err := tomcat.FindPid(param.ContainerPath)
	if err != nil {
		utils.Logger().Warn("exec cmd:%v fail:%v", cmd, err)
		return err
	}
...
func (tomcat TomcatType) StartCmd(param *model.StartApp, startCmd string) error {
	...
	f.Close()
	err = utils.ExecCommandAsync(startCmdSh) //针对tomcat移步调用
	return err
}
func (tomcat TomcatType) FindPid(packageName string) (string, error) {
...
func (tomcat TomcatType) Clear(param *model.StartApp) error {
...
func (tomcat TomcatType) MvFile(param *model.StartApp) error {
...
func (tomcat TomcatType) HandleAction(param *model.StartApp) error {
	switch param.Action {
	case "restart":
		return tomcat.ReStart(param)
	case "stop":
		return tomcat.Stop(param)
	default:
