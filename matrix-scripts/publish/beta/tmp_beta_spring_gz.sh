...
if [ "${build_type}" != "release" ];then
    source_path="${WORKSPACE}${deploy_path}"
    cd "${WORKSPACE}${deploy_path}"
else
    source_path="${WORKSPACE}${PUBLISH_RECORD}"/"${deploy_path}"
