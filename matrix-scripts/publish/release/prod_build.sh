...
function push_jar_server(){
    ...
    do
        ...
        if test -z "${off_http_status_code}" ;then
            error_exit "${ip} ${jar_name} 摘流量失败, ${off_http_response}" '' 4 9
        fi
        if [ "${off_http_status_code}" -ne "200" ];then
            error_exit "${ip} ${jar_name} 摘流量失败, ${off_http_status_code}" '' 4 9
        else
            ...
            if test -z "${off_response_code}" ;then
                error_exit "${ip} ${jar_name} 摘流量失败, ${off_response_body}" '' 4 9
            fi
            off_response_message=`getValue "${off_response_body}" "message"`
            msg "${ip} ${jar_name} ${off_response_message}"
            if [ "${off_response_code}" != "0" ];then
                error_exit "${ip} ${jar_name} 摘流量失败, ${off_response_message}" '' 4 9
            fi
            success "${ip} ${jar_name} 摘流量成功! ${off_response_message}"
        fi
        #2、启动（发启动消息；获取启动结果）
