...
function mvn_build(){
        ...
        if test -z ${MAVEN_PATH} ;then
                if test -z  ${mvn_ops} ;then
                        exec_cmd "mvn clean install -U -Dmaven.test.skip=true -P ${GROUP} -f ${PROJECT_POM} " 14 14
                else
                        exec_cmd "mvn clean install -U -Dmaven.test.skip=true -P ${GROUP} -f ${PROJECT_POM} ${mvn_ops}" 14 14
                fi
        else
                if test -z  ${mvn_ops} ;then
                        exec_cmd "mvn clean install -U -Dmaven.test.skip=true -P ${GROUP} -f ${PROJECT_POM} -s ${MAVEN_PATH} " 14 14
                else
                        exec_cmd "mvn clean install -U -Dmaven.test.skip=true -P ${GROUP} -f ${PROJECT_POM} -s ${MAVEN_PATH} ${mvn_ops}" 14 14
                fi
        fi
        if [ "${build_type}" == "release" ];then
                DEPLOY_STATUS=13
...
#        local off_http_status_code=`echo ${off_http_response} | awk -F"}" '{print $2}'`
        local off_http_status_code=`echo ${off_http_response:0-3:3}`
        echo "off_http_response:"${off_http_response}
        echo "off_http_status_code:"${off_http_status_code}
        if test -z "${off_http_status_code}" ;then
            error_exit "摘流量失败, ${off_http_response}" '' 4 9
        fi
        if [ "${off_http_status_code}" -ne "200" ];then
            error_exit "摘流量失败, ${off_http_status_code}" '' 4 9
        else
            local off_response_body=`echo ${off_http_response} | awk -F"}" '{print $1}'`
            local off_response_code=`getValue "${off_response_body}" "code"`
            if test -z "${off_response_code}" ;then
                error_exit "摘流量失败, ${off_response_body}" '' 4 9
            fi
            off_response_message=`getValue "${off_response_body}" "message"`
            if [ "${off_response_code}" != "0" ];then
                error_exit "摘流量失败, ${off_response_message}" '' 4 9
            fi
        fi
        msg "${off_response_message}"
...
#        local healthcheck_url=http://${ip}:${health_check}
        ...
        local post_param=\{\"buildType\":\"${build_type}\",\"packageType\":\"${APP_TYPE}\",\"healthCheckUrl\":\"${healthcheck_url}\",\"packageName\":\"${app_name_zip}\",\"packagePath\":\"${PACKAGE_PATH}\",\"containerPath\":\"${CONTAINER_PATH}\",\"packageUrl\":\"${package_url}\",\"action\":\"restart\",\"startOps\":\"${START_OPS}\",\"recordId\":\"${PUBLISH_RECORD}\",\"ip\":\"${ip}\",\"md5Name\":\"${md5_file}\"\}
#        local post_param=\{\"buildType\":\"${build_type}\",\"packageType\":\"${APP_TYPE}\",\"packageName\":\"${package_name}\",\"packagePath\":\"${PACKAGE_PATH}\",\"containerPath\":\"${CONTAINER_PATH}\",\"packageUrl\":\"${package_url}\",\"action\":\"restart\",\"startOps\":\"${START_OPS}\",\"recordId\":\"${PUBLISH_RECORD}\",\"ip\":\"${ip}\",\"md5Name\":\"${md5_file}\"\}
...
#        local http_status_code=`echo ${http_response} | awk -F"}" '{print $2}'`
        ...
        echo "${ip} ${jar_name} http_response:"${http_response}
        echo "${ip} ${jar_name} http_status_code:"${http_status_code}
...
#                    msg "${ip} ${jar_name} 启动完成"
        ...
        echo "${ip} ${jar_name} on_http_response:"${on_http_response}
        echo "${ip} ${jar_name} on_http_status_code:"${on_http_status_code}
        if test -z "${on_http_status_code}" ;then
            error_exit "${ip} ${jar_name} 挂流量失败, ${on_http_response}" '' 4 9
        fi
        if [ "${on_http_status_code}" -ne "200" ];then
            error_exit "${ip} ${jar_name} 挂流量失败, ${on_http_status_code}" '' 4 9
        else
            ...
            if test -z "${on_response_code}" ;then
                error_exit "${ip} ${jar_name} 挂流量失败, ${on_response_body}" '' 4 9
            fi
            on_response_message=`getValue "${on_response_body}" "message"`
            if [ "${on_response_code}" != "0" ];then
                error_exit "${ip} ${jar_name} 挂流量失败, ${on_response_message}" '' 4 9
            else
                DEPLOY_STATUS=8
                ipCallBack &>/dev/null
                success "${ip} ${jar_name} 挂流量成功, ${on_response_message}"
                break
            fi
        fi
    done
