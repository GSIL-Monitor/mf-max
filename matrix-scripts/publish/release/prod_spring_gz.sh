...
function backup(){
        ...
        exec_cmd "cp ${source_path} ${backup_file_path}/${rtag_name}-${GROUP}" 4 9
}
function rollback(){
        backup_file_path=/data/backup/${jar_name}
...
function rm_online_file(){
    ssh www@$ip "rm $online_file_zabbix"
}
...
startLogPath=`getValue "${STRING}" "startLogPath"`
echo "startLogPath:${startLogPath}"
startSuccFlag=`getValue "${STRING}" "startSuccFlag"`
echo "startSuccFlag:${startSuccFlag}"
DEPLOY_STATUS=
TAG=
startShellPath=`getValue "${STRING}" "startShellPath"`
echo "startShellPath:"${startShellPath}
stopShellPath=`getValue "${STRING}" "stopShellPath"`
echo "stopShellPath:"${stopShellPath}
...
#        local healthcheck_url=http://${ip}:${health_check}
        ...
        local post_param=\{\"buildType\":\"${build_type}\",\"packageType\":\"${APP_TYPE}\",\"healthCheckUrl\":\"${healthcheck_url}\",\"packageName\":\"${package_name}\",\"packagePath\":\"${package_path}\",\"packageUrl\":\"${package_url}\",\"action\":\"restart\",\"startOps\":\"${start_ops}\",\"recordId\":\"${PUBLISH_RECORD}\",\"ip\":\"${ip}\",\"md5Name\":\"${md5_file}\",\"stopShellPath\":\"${stopShellPath}\",\"startShellPath\":\"${startShellPath}\",\"appLogPath\":\"${startLogPath}\",\"appLogKeyWord\":\"${startSuccFlag}\"\}
