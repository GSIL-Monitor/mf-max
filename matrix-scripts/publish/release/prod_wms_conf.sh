...
{
    ((IND_SERVER++))
    if [ ${IND_SERVER} -gt 1 ]; then
        #echo "$IND_SERVER sleep 1m"
        #sleep 1m
        echo "no sleep"
    fi
}
...
function push_js_server(){	
	...
	do
		#sleep_fun
        #/usr/bin/ssh -t -t  www@${ip} "sudo mkdir -p ${deploy_path}"  
		#msg "服务停止中......"
		#/usr/bin/ssh -t -t  www@${ip} "sudo service t1 stop"
		#/usr/bin/ssh -t -t www@${ip} "sh /home/www/server/stopallserver.sh"
		#if [ "$?" != "0" ]; then
        #	error_exit "命令执行失败: 错误码为 $?" '' 4 9
    	#fi
		#msg 't1 停止'
    	...
    	#fi
		#msg "服务启动中......"
		#/usr/bin/ssh -t -t  www@${ip} "sudo service ${WMS_SERVICE} start"
		#/usr/bin/ssh -t -t www@${ip} "sh /home/www/server/startallserver.sh"
		#if [ "$?" != "0" ]; then
        #	error_exit "命令执行失败: 错误码为 $?" '' 4 9
    	#fi
		#echo 't1 启动'
...
#        local healthcheck_url=http://${ip}:${health_check}
        local post_param=\{\"buildType\":\"${build_type}\",\"packageType\":\"${APP_TYPE}\",\"healthCheckUrl\":\"${healthcheck_url}\",\"packageName\":\"${package_name}\",\"packagePath\":\"${package_path}\",\"packageUrl\":\"${package_url}\",\"action\":\"restart\",\"startOps\":\"${START_OPS}\",\"recordId\":\"${PUBLISH_RECORD}\",\"ip\":\"${ip}\",\"md5Name\":\"${md5_name}\",\"wmsJsService\":\"${WMS_SERVICE}\"\}
